...
void mr_iptv_free(mr_iptv_directory *d) { free(d->channels); mr_iptv_init(d); }
...
{
    unsigned i; int category = 0, search = 0;
    if (!c || c->is_nsfw || c->closed || !c->stream_count) return 0;
    if (f && f->country && *f->country && strcmp(f->country, "All") &&
        strcmp(c->country, f->country)) return 0;
    if (f && f->category && *f->category && strcmp(f->category, "All")) {
        for (i = 0; i < c->category_count; i++)
            if (!strcmp(c->categories[i], f->category)) category = 1;
        if (!category) return 0;
    }
    if (!f || !f->search || !*f->search) return 1;
...
size_t mr_iptv_filter_channels(const mr_iptv_directory *d, const mr_iptv_filter *f,
                               size_t *indices, size_t capacity)
{
    size_t i, n = 0;
    for (i = 0; i < d->channel_count; i++) if (mr_iptv_channel_visible(&d->channels[i], f)) {
        if (n < capacity) indices[n] = i;
        n++;
    }
    return n;
}
...
{
    const char *p;
    if (!u || (strncmp(u, "http://", 7) && strncmp(u, "https://", 8))) return 0;
    p = strstr(u, "://") + 3;
